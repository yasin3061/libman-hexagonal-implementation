...
public class EmailFacade implements SendReservationConfirmation {
    ...
    public void handle(SendReservationConfirmationCommand sendReservationConfirmation) {
        String bookTitle = database
                ...
                .orElseThrow(() -> new IllegalArgumentException("Can't get book title from " +
                        "database. Reason: there is no book with an id: " + sendReservationConfirmation.getBookId()));
        String userEmailAddress = database
                .getUserEmailAddress(sendReservationConfirmation.getUserId())
                .orElseThrow(() -> new IllegalArgumentException("Can't get email address from " +
                        "database. Reason: there is no user with an id: " + sendReservationConfirmation.getUserId()));
